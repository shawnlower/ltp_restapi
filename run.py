...
def main():
    ...
    parser.add_argument('--config', help='Config file with application '\
                        'settings (default: {})'.format(settings.DEFAULT_CONFIG))
    parser.add_argument('-e', '--environment', help='Set the environment '\
            '(e.g. testing). This uses the config overrides (core:testing). '\
            ' [NOTE]: You should NOT use this to separate prod/dev in the'\
            ' same file!')
